...
The method above is not efficient: in the last example we generated `r format(n_sims, big.mark = ",", scientific = FALSE)` samples but then used only `r format(nrow(posterior), big.mark = ",", scientific = FALSE)` of them by conditioning on the actual results for the banner ad. It was easy to code, but with bigger datasets and/or more complicated models, we need something more efficient. To do this we need some probability theory.

## Probability Theory Recap

Calculating likelihoods is important, can use the `d*()` functions, such as `dbinom()`, to calculate the probability of specific values for any distribution.

```{r}
# Calculate the likelihood of getting 13 clicks on 100 ads with 0.1 prob of a 
# click on each ad using the old method
n_ads_shown <- 100
proportion_clicks <- 0.1
n_visitors <- rbinom(n = 99999, 
                     size = n_ads_shown, 
                     prob = proportion_clicks)
prob_13_visitors_old <- mean(n_visitors == 13)
prob_13_visitors_old
# Then again using dbinom()
prob_13_visitors_new <- dbinom(x = 13, size = 100, prob = 0.1)
prob_13_visitors_new
```

Can now use these techniques to tackle the problem from the previous section, but using exact calculations instead of simulation.

```{r}
# Number of ads is known for sure
n_ads_shown <- 100

# Number of visitors and proportion of clicks are to be estimated, so we set 
# these to all possible values (within reason)
n_visitors <- seq(0, 100)
proportion_clicks <- seq(0, 1, by = 0.01)
pars <- expand.grid(n_visitors = n_visitors, 
                    proportion_clicks = proportion_clicks) %>% 
    as_tibble() %>% 
    # Add the prior using dbeta() (although the course used dunif())
    mutate(prior = dbeta(x = proportion_clicks, 
                         shape1 = 10, 
                         shape2 = 90)) %>% 
    # Add the likelihood using dbinom
    mutate(likelihood = dbinom(x = n_visitors, 
                               size = n_ads_shown, 
                               prob = proportion_clicks)) %>% 
    # Add unstandardised probability
    mutate(probability = prior * likelihood) %>% 
    # Standardise the probability
    mutate(probability = probability/sum(probability))

# Now condition on getting 13 clicks, i.e. drop other data
posterior <- pars %>% 
    filter(n_visitors == 13) %>% 
    # Restandardise the probability
    mutate(probability = probability/sum(probability))

# Visualise
posterior %>% 
    filter(proportion_clicks < 0.4) %>% 
...
pars %>% 
    filter(proportion_clicks <= 0.3, 
           n_visitors <= 30) %>% 
    ggplot(aes(x = proportion_clicks, y = n_visitors)) + 
    geom_raster(aes(fill = probability))
```

# More Parameters, More Data, More Bayes

A new problem: we want to have a party at the lake on 20 July, but only if the water temperature is high enough (say, 18&deg;C). We have data for the last five years on 20 July: `c(19, 23, 20, 17, 23)`. The binomial distribution is no longer sufficient, but we can try out a normal distribution, for example $Temp \sim \mathcal{N}(\mu = 20, \sigma = 2)$.

```{r}
# Save the previous temperatures
temp <- c(19, 23, 20, 17, 23)

# Try some random draws from Normal(20, 2)
rerun(5, 
      rnorm(5, 20, 2))
```

The random draws seem that they could plausibly have been generated by the same process that generated our data. Given this set of parameters, $\theta$, and set of data, $x_i, i \in \{1, 2, 3, 4, 5\}$, we can calculate $P(x_i|\theta)$.

```{r}
# Generate the individual likelihoods
like <- dnorm(temp, 20, 2)
like

# Multiply them to get the probability of this exact set of data
prod(like)

# The numbers are now unmanageably small, so work on the log scale instead
log(like)

# Which means summing the log-likelihoods, instead of multiplying
sum(log(like))

# check the maths, should be about zero
exp(sum(log(like))) - prod(like)
```

Now define a model. Firstly the temperature:

$$
temp_i \sim \mathcal{N}(\mu, \sigma^2)
$$

What about priors for $\mu$ and $\sigma$?

$$
\sigma \sim \text{Uniform}(0, 10)
\\
\mu \sim \mathcal{N}(18, 5^2)
$$

Use grid approximation as before.

```{r}
# Temperatures measured are known
temp <- c(19, 23, 20, 17, 23)

# Mu and sigma are to be estimated, so we set these to all possible values 
# (within reason)
mu <- seq(8, 30, by = 0.5)
sigma <- seq(0.1, 10, by = 0.25)
pars <- expand.grid(mu = mu, 
                    sigma = sigma) %>% 
    as_tibble() %>% 
    # Add the priors
    mutate(prior_mu = dnorm(mu, 18, 5), 
           prior_sigma = dunif(sigma, 0, 10)) %>% 
    mutate(prior = prior_mu * prior_sigma) %>% 
    # Add the likelihood 
    mutate(likelihood = map2_dbl(mu, 
                                 sigma, 
                                 ~ prod(dnorm(temp, .x, .y)))) %>% 
    # Add unstandardised probability
    mutate(probability = prior * likelihood) %>% 
...
Now to answer the question of whether to have a party we can draw samples from a normal distribution using parameters weighted by the probabilities from the posterior distribution.

```{r}
sample_indices <- sample(seq_along(pars$probability), 
                         size = 1e5, 
                         replace = TRUE, 
                         prob = pars$probability)
samples <- pars[sample_indices, c("mu", "sigma")] %>% 
    mutate(sim_temp = map2_dbl(mu, sigma, ~ rnorm(1, .x, .y)))

# Visualise the distribution of mu and sigma, then of the simulated temperatures
samples %>% 
    select(mu, sigma) %>% 
    gather() %>% 
    ggplot(aes(x = value)) + 
...
samples %>% 
    ggplot(aes(x = sim_temp)) + 
    geom_histogram(binwidth = 0.5) + 
    theme_light()

# What's the probability of the temp being 18 or above?
mean(samples$sim_temp >= 18)
```

There's a `r round(mean(samples$sim_temp >= 18), 2) * 100`% chance of the water being warm enough, so probably safe to plan the party.

## Using BEST

John Kruschke's[2] `BEST` package helps with building models more quickly. Can tackle the water temperature problem with it.
...
```{r}
# Create a model with just the BESTmcmc function, inspect the output
fit <- BESTmcmc(temp)
fit
# Default plot method available
plot(fit)
```

BEST stands for _Bayesian Estimation Supersedes the t-test_: it can be used as the t-test for comparing two datasets. Suppose we have two sets of IQ measurements from two zombie groups and want to compare them.

```{r}
iq_brains <- c(44, 52, 42, 66, 53, 42, 55, 57, 56, 51)
iq_regular <- c(55, 44, 34, 18, 51, 40, 40, 49, 48, 46)
# Can simply calculate the difference in sample means
mean(iq_brains) - mean(iq_regular)

# Then fit a BEST model to both sets at once
best_posterior <- BESTmcmc(iq_brains, iq_regular)

# Inspect and plot the output
best_posterior
plot(best_posterior)
```

[1]: https://www.datacamp.com/courses/fundamentals-of-bayesian-data-analysis-in-r
[2]: https://sites.google.com/site/doingbayesiandataanalysis/